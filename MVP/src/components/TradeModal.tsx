...
export function TradeModal({ 
  ...
  const cacheRef = useRef<Map<string, { ts: number; items: Array<{ ticker: string; companyName: string; price: number }> }>>(new Map());
  ...
  // Debounced live search (Yahoo via RapidAPI) with host fallback and local fallback
  useEffect(() => {
    ...
    const timeoutId = setTimeout(async () => {
      try {
        ...
        if (!apiKey) {
          ...
          setError('Missing RapidAPI key. Add REACT_APP_RAPIDAPI_KEY to MVP/.env and restart.');
          return;
        }
        ...
        const tryFetch = async (baseUrl: string, host: string) => {
          ...
          const mapped: Array<{ ticker: string; companyName: string; price: number }> = (quotesJson?.quoteResponse?.result || [])
            .map((r: any) => ({
              ticker: r.symbol,
        ...
        // Lock to apidojo host to match your subscription, with minimal backoff on rate limit
        let mapped: Array<{ ticker: string; companyName: string; price: number }> = [];
        try {
          mapped = await tryFetch('https://apidojo-yahoo-finance-v1.p.rapidapi.com', 'apidojo-yahoo-finance-v1.p.rapidapi.com');
        } catch (e) {
        ...
        if (mapped.length === 0) {
          ...
          const local = mergedLocal
            ...
            .slice(0, 5)
            .map(s => ({ ticker: s.ticker, companyName: s.companyName, price: s.price }));
          setResults(local);
          setShowDropdown(true);
          setIsSearching(false);
          cacheRef.current.set(q.toLowerCase(), { ts: Date.now(), items: local });
          return;
        }
        ...
        setShowDropdown(true);
        setIsSearching(false);
        cacheRef.current.set(q.toLowerCase(), { ts: Date.now(), items: mapped });
      } catch {
        setIsSearching(false);
        ...
        setShowDropdown(true);
        if (local.length === 0) {
          setError('No matches found (API blocked or rate-limited). Try AAPL, MSFT, TSLA, AMZN, NVDA.');
        }
      }
    }, 300);
